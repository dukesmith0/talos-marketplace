...
# /talos-add — Store Knowledge

## Step 0
Run `talos vault` via Bash. Read `_brain/schemas.yaml`.
...
## Step 2: Classify
Dispatch `talos-brain:classifier` agent → type, confidence, folder.
...
### Multi-Topic Split
3+ distinct topics → dispatch `talos-brain:splitter` agent → separate notes, each with own 5 Ws, cross-linked.

### Conflict Detection (facts only)
Search vault for same topic. If contradicting fact: show both, ask user. Log unresolved to `_brain/conflicts.md`.
