...
# /loci-ideate — Idea Generation with Perspective Review
...
## Phase 2: Evaluate
Dispatch `loci-thoughts:perspective-reviewer` agent with the ideas to score from 3 perspectives (1-10 each):
...
**Hub update**: for each tag with a hub page in `tags/`, append new idea to hub's `## Connections`. If a tag appears 3+ times with no hub → auto-create via `/loci-hub`.

## Activity Log
Run via Bash: `loci log "ideate: <brief outcome>"`
